...
class LinkedIn  
{
    ...
    public function __construct(string $app_id, string $app_secret, string $callback, string $scopes, bool $ssl = true)
    {
        ...
        $this->scopes =  $scopes;
        $this->csrf = random_int(1000000,999999999);
    ...
    public function getEmail($accessToken)
     {
        // echo "Hola";
        // // exit();
        $url = "https://api.linkedin.com/v2/clientAwareMemberHandles?q=members&projection=(elements*(primary,type,handle~))&oauth2_access_token=".$accessToken;
        $params = [];
